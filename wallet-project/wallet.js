...
async function main() {
    ...
    if (process.argv.length > 2) {
        ...
        console.log(`Usage: node wallet.js list node wallet.js add natuni Admin. Not enough arguments.`)
        return;
    }
...
function readCertCryptogen(org, user) {
    var certPath = CRYPTO_CONFIG_PEER_ORGS + "/" + org + ".edu/users/" + user + "@" + org +
        ".edu/msp/signcerts/" + user + "@" + org + ".edu-cert.pem"
...
function readPrivateKeyCryptogen(org, user) {
    var pkFolder = CRYPTO_CONFIG_PEER_ORGS + "/" + org + ".edu/users/" + user + "@" + org + ".edu/msp/keystore";
    var pkfile;
